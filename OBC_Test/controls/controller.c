...
static uint8_t first_B = 2, light, w_ctrl = 1;
...
void apply_torque(vector v_m)
{
  ...
  char buf[100];
  sprintf(buf,"%u %u %u\r",  Current_state.pwm.x, Current_state.pwm.y, Current_state.pwm.z);
  send_preflight(buf, strlen(buf));
  
  if(v_m[0] < 0)
  {
    Current_state.pwm.x_dir = 1;
    Current_state.pwm.x = 0xFFFF - Current_state.pwm.x;
  }
  else
  ...
  if(v_m[1] < 0)
  {
    ...
    Current_state.pwm.y = 0xFFFF - Current_state.pwm.y;
  }
  else
  ...
  if(v_m[2] < 0)
  {
    ...
    Current_state.pwm.z = 0xFFFF - Current_state.pwm.z;
  }
  else
...
void control(void){
  ...
  pwm_tmp = Current_state.pwm;
  reset_PWM();
  Current_state.pwm = pwm_tmp;
  ...
  if(((GPS_done == 0) && (Time % 600 == 0)) || ((GPS_done < 0) && (!first_B) && (vector_norm(v_m_D) < 0.009 )))
  {
    ///* Switch on the GPS First
    send_preflight("Read GPS\r", 9);
    read_GPS();
  }
  /// Check if in Nominal or Detumbling Mode
  if(GPS_done >= 0)
  {
    send_preflight("I'm inside\r", 11);
    if(GPS_done > 0)
    {
      copy_gps_reading();
      send_preflight("Copy GPS\r", 9);
    }
    read_SS();
    ...
    sprintf(buf, "W = %f, light = %d, w_ctrl = %d\r", norm_w, light, w_ctrl);
    send_preflight(buf, strlen(buf));
  ...
  if(Mode == DETUMBLING)
  {
    send_preflight("Detumbling\r", 11);
    apply_torque(v_m_D);
  }
  
  else if(Mode == NOMINAL)
  {
	send_preflight("Nominal\r", 8);
    apply_torque(v_m_N);
  }
